...
* `rqutils/product.py` -- not on this branch. Its `Solution.lower_bound` is *not* a lower bound on
  ...
  recorded in `markdowns/sdp-lower-bound.md`, which is where the measurement lives.

Method: two independent sweeps -- one over the library source, one over the ~4k lines of findings
under `markdowns/` plus `CLAUDE.md`/`NOTES.md` -- then hands-on verification of the severe items by
...
* `apply_h`'s positional `(scanned, cache_level)` form was **deleted** rather than deprecated,
  ...
  (`markdowns/rqutils-requests.md` C1).
...
`markdowns/rqutils-precond-request.md` paired `bit q -> column q` with an `xxz` operator built by
...
accepted. `markdowns/rqutils-requests.md` concedes this: "That residue is much smaller... but it is not
zero."
...
returns it as "Calculated ground state energy" with no indication. `markdowns/locg.md` records that this
...
wider row and aliases distinct states", and `markdowns/scaling-pocs.md` calls the `n+1 <= 64` limit "a hard
...
From `CLAUDE.md`, `NOTES.md`, `markdowns/rqutils-requests.md`, `markdowns/skqd.md`, `markdowns/locg.md`,
