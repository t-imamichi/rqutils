...
paper `markdowns/locg-chebyshev-prefilter.md` §7 names as the prefilter's provenance, now also cited as `[5]`
...
- **`markdowns/deflation-preconditioner.md`** — a *two-level / deflation preconditioner*, tested 2026-08-28
  ...
  sign, with `κ`).
- **`markdowns/locg-chebyshev-prefilter.md`** — the single-vector prefilter that shipped, `prefilter=(32, 2)`,
...
narrow projected spectrum? — is already answered in `markdowns/locg-chebyshev-prefilter.md` §3.1, which swept
