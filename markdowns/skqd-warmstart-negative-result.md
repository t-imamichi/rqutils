...
(`markdowns/skqd-sqd-solve-tolerance.md` §8); the refined shape was measured on 2026-09-17 and produced **no
...
(`markdowns/locg-next-candidates.md` §1.4 records it as a standing constraint on any candidate).
...
does depend on which degenerate member gets picked — see `markdowns/rqutils-requests-2.md`). Measured:
...
3. **There is no seam, and `rqutils` will not add one.** `sqd()` does not accept a start vector; it is
   ...
   deterministic function of the subspace (`markdowns/rqutils-requests-2.md`; an early version gave 5
...
`markdowns/skqd-sqd-solve-tolerance.md` §8.
