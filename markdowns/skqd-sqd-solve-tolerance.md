...
`markdowns/skqd-basis-opt-optimization.md`, which is also outbound.
...
`CLAUDE.md` and `markdowns/locg.md`, that per-iteration work is load-bearing — seven defects (I1–I7) that
...
- **Eigenvector continuation across the growing subspace.** Both `sqd` driver loops grow the subspace
  ...
  **Re-tested 2026-09-17: this rejection stands, but the stated mechanism did not reproduce.**
  `poc/warmstart.py` tried the shape the mechanism implies — carry the eigenvector on
...
- **Jacobi preconditioner on the raw indefinite projected `H`.** `ground_locg(precond=...)` shipped
  ...
  `markdowns/rqutils-precond-request.md` closing that line.
...
- **Randomized `lambda_max` estimate to enable a valid Jacobi preconditioner.** The premise was that
  `markdowns/rqutils-precond-request.md` closed the shift line for want of a *lower* bound on `H`, whereas a
...
- **A finer `states_size` bucket ladder than the power of two.** `uniquify_states` pads to the next
  ...
  So **C3's power-of-two default is correct**, for the reason `markdowns/rqutils-requests.md` already gives:
...
- **Randomized block Krylov / block Lanczos in place of block-1 LOBPCG** — the SOTA direction
  ...
  *problem's spectral gap*, not solver slack: all three long solves converge correctly (energies match
  ...
  (preconditioning, closed in `markdowns/rqutils-precond-request.md`).
