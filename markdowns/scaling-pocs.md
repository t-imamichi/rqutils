...
Six ideas for improving `sqd`'s scaling, each built as a POC under `poc/` and measured.
...
is an error this repo has hit before -- a result on one backend says nothing about another, in either
...
in `poc/gpu_unverified.py` are now settled and one remains unrun:
...
- **Multi-GPU speedup: UNRUN.** The box has one GH200. `--devices` sets `CUDA_VISIBLE_DEVICES`, a
  *filter* over devices the driver already exposes, so it cannot conjure a second GPU — this is unrun,
  not unresolved, and needs a physically multi-GPU machine. `poc/sharding.py` already covers sharding
...
is precisely what made it unalarming. Both files now time against `searchsorted.xsource_sort_legacy`, a
...
**This is now what `get_xsource` does**; `poc/searchsorted.py` remains as the exploratory record. The
...
`poc/searchsorted.py`. `S` comes out of `uniquify_states` already lex-sorted, so finding
...
`poc/caching.py`. The predicted memory saving holds **exactly**: 15.99× (complex), 8.00× (real),
...
`poc/real_symmetric.py`. Checked the premise before building: `apply_h` **already** propagates
...
`poc/mixed_precision.py`. The per-iteration win is real (1.29–1.74× matvec-only, 1.23–1.24× at fixed
...
`poc/graycode.py`. The composition `A2 = Ad ∘ A1` is only valid when the **intermediate** state
...
`poc/sharding.py` uses `XLA_FLAGS=--xla_force_host_platform_device_count=4` to get virtual CPU
...
2. **Keep the sharding fix** and consider promoting `poc/sharding.py` into `tests/` under the
...
rank-select `cap` in `markdowns/rqutils-multiobs-response.md` §5.3, which had no detectable failure.
