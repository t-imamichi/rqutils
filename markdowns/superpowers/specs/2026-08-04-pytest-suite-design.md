...
audit that motivated that rewrite, `markdowns/locg.md`, enumerates twelve specific ways the previous
...
Tests are organized **by defect**, keyed to `markdowns/locg.md` items, rather than by public API surface
...
`markdowns/locg.md` closes with a methodological warning that shapes this design:
...
A bug found while validating this design, not present in `markdowns/locg.md`.
...
- **Sharded / multi-device execution.** `markdowns/locg.md` lists this as unverified, and the
...
- **Tight spectral clusters** below ~1e-10 relative gap. `markdowns/locg.md` documents the residual
