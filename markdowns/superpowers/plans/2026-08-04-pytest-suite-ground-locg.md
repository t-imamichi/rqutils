...
**Goal:** Establish the repository's first pytest suite, covering `rqutils/ground_locg.py` with tests keyed to the defects in `markdowns/locg.md`, and fix a mixed-dtype crash found while designing it.
...
suite depends on it. Tests are organized by defect and keyed to the audit items in `markdowns/locg.md`;
