...
The task was to verify `markdowns/rqutils-multiobs-request.md` — a drafted change request against
...
`poc/ooc_uniquify.py` bounds the sort's working set by chunking, but bails out at
...
### 4.3 Range-partitioned sample sort — prototype (`poc/range_partition.py`)
...
**The partial-J dial is the answer**, and `markdowns/scaling-pocs.md` §2 already scoped it correctly: *"only
...
Related: `markdowns/rqutils-multiobs-request.md` (the request), `markdowns/rqutils-multiobs-response.md` (the
reply, from the `rqutils` side), `markdowns/scaling-pocs.md` §11, `NOTES.md` under the `N ≤ 2^31 - 1` ceiling.
