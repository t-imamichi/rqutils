...
**Direction — note this differs from its neighbours.** `markdowns/rqutils-requests.md` and
`markdowns/rqutils-precond-request.md` are *inbound*: written from the `spinchain` side, asking `rqutils`
...
2. **`markdowns/rqutils-requests.md` is evidence-first by construction.** Every item there states the
