...
Reply to `markdowns/rqutils-tol-request.md`, from the `rqutils` side. Branch `dev`, version still `0.2.0`
(unreleased).
...
> ```
>
> satisfying **either** arm. See `markdowns/rqutils-atol-rtol-request.md` and its response for the reasoning;
...
deliver a criterion other than the one requested — the same reasoning as the `poc/hash_partition_jax` capacity note.
...
- **`poc/sharding`**: sharded and single-device agree at all six `cache_level`s, worst deviation
