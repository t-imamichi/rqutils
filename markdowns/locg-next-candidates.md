...
branch `locg-chebyshev`, after `markdowns/locg-chebyshev-prefilter.md` shipped. Records what a literature
...
`ground_locg` should wait for the GPU numbers (`poc/prefilter_gpu.py`), which could
...
see `markdowns/skqd-sqd-solve-tolerance.md` §8 for the seven measured rejections.
...
3. **Must attack the spectral gap** — per-iteration work is ~75% `O(N)` safeguards (≈94
   dot-equivalents), already inside one `@jax.jit` with a `lax.while_loop`, and `markdowns/locg.md`
...
- **Polynomial-preconditioned eigensolvers** (Embree/Loe/Morgan line; also the ChFSI literature
  already cited in `markdowns/locg-chebyshev-prefilter.md` §7). This is the published formalization of
...
**This is the depleted-residual failure again**, in a new place. `markdowns/skqd-sqd-solve-tolerance.md` §8
...
mixed signs on an indefinite operator destroy descent (0.29–0.35×). `markdowns/rqutils-precond-request.md`
...
`markdowns/skqd-sqd-solve-tolerance.md` §8 look mixed: **a host-side prototype of a device-resident algorithm
