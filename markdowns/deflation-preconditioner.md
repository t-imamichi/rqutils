...
This was the last untried deterministic candidate. `markdowns/rqutils-precond-request.md:697` named it — "a
...
viable *lower* bound on the projected operator's minimum. `markdowns/rqutils-precond-request.md:786` then
...
that `markdowns/skqd-sqd-solve-tolerance.md` §7.1 warns invalidates every iteration-count conclusion.
...
matvec-to-`O(N)` ratio is exactly what shifts on GPU — the reason `poc/prefilter_gpu.py` exists unrun.
...
With this, every deterministic candidate in `markdowns/locg-next-candidates.md` and the preconditioning line
in `markdowns/rqutils-precond-request.md` is measured and rejected. Combined with the randomized families
...
- `poc/prefilter_gpu.py` — written, unrun, needs CUDA. It re-prices several
