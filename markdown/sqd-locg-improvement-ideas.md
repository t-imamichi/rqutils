...
**Gated measurement is in: DO NOT INTEGRATE (2026-09-05, anchored 2026-09-07).** `poc/sqd_multinode` ran on real
...
be built until that is measured. See `NOTES.md`, "poc/sqd_multinode anchored".
...
**Promoted 2026-09-05 by the `poc/sqd_multinode` multi-node run, reinforced by its 2026-09-07 anchor**, which
...
algorithmic change. See `markdown/improvement-ideas-2026-09-25.md` section 1.
...
load-bearing for the mutation and residual-honesty investigations recorded in `test/test_ground_locg.py`
and `NOTES.md`.
...
3. **Section 8, rescoped -- make the norms combinable.** *(First instead, if the target is multi-node:
   `poc/sqd_multinode` measured 4 devices at 4.06x slower than 1, with the first hop dearer than the second.)* Do not
