...
branch `locg-chebyshev`, after `markdown/locg-chebyshev-prefilter.md` shipped. Records what a literature
...
see `markdown/skqd-sqd-solve-tolerance.md` §8 for the seven measured rejections.
...
3. **Must attack the spectral gap** — per-iteration work is ~75% `O(N)` safeguards (≈94
   dot-equivalents), already inside one `@jax.jit` with a `lax.while_loop`, and `markdown/locg.md`
...
- **Polynomial-preconditioned eigensolvers** (Embree/Loe/Morgan line; also the ChFSI literature
  already cited in `markdown/locg-chebyshev-prefilter.md` §7). This is the published formalization of
...
**This is the depleted-residual failure again**, in a new place. `markdown/skqd-sqd-solve-tolerance.md` §8
...
`markdown/skqd-sqd-solve-tolerance.md` §8 look mixed: **a host-side prototype of a device-resident algorithm
