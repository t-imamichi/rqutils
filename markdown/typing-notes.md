...
`rqutils/ test/ examples/` with all four extras installed.
...
- **Match the real parameter kind.** `sqd`'s flags are keyword-only, `ground_locg`'s are
  ...
  because `poc/real_symmetric` and `poc/mixed_precision` pass `maxiter` positionally.
...
Enabling it reports **9 findings** (5 in `rqutils/`, 2 in `poc/`), and at least one is a
...
uv run --extra dev --extra qiskit --extra mpl --extra qutip \
  ty check -c 'rules.<name>="error"' rqutils/ test/ examples/ poc/
```
