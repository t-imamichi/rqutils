...
* `rqutils/product.py` -- not on this branch. Its `Solution.lower_bound` is *not* a lower bound on
  ...
  recorded in `markdown/sdp-lower-bound.md`, which is where the measurement lives.

Method: two independent sweeps -- one over the library source, one over the ~4k lines of findings
under `markdown/` plus `CLAUDE.md`/`NOTES.md` -- then hands-on verification of the severe items by
...
(qubit `q` is bit `q` of the state-vector index). `test/conftest.py:124` pins the `sqd` side
(`unique.dot(1 << np.arange(num_qubits)[::-1])`); `test/test_svsim.py:249` pins the other.
...
spectrum of a *different* operator (`test/test_math.py:153` pins the error at `> 1.0`).
...
scripts under `poc/` -- i.e. exactly the code that pushes `N`. `NOTES.md` records that
...
returns it as "Calculated ground state energy" with no indication. `markdown/locg.md` records that this
...
but a **single** filler row is still strictly increasing and passes (`test/test_sqd.py:397` records
...
wider row and aliases distinct states", and `markdown/scaling-pocs.md` calls the `n+1 <= 64` limit "a hard
...
`test/test_sqd.py::TestPackedStatesInput::test_single_qubit_is_the_one_width_the_flag_cannot_check`,
