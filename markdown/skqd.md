...
> despite being listed in the docstring. `test/test_svsim.py` covers all of it -- 67 tests,
...
- **`rqutils/sqd.py:544`** (`iterm & 255` where `iterm & 7` is intended) only affects
  ...
  along with two initial-vector bugs that did affect the default path; see `test/test_sqd.py`.)*
