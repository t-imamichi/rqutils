...
>
> Verifying C3 on a mesh required running `poc/sharding.py`, which **failed at stock
...
> green** while the mesh path raises, because pytest was single-device only and `poc/sharding.py` was
> the sole coverage. `test/test_sqd.py::TestShardedDiagonalRank` now closes that gap, running
