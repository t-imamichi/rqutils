...
Cache `J'` of the `J` groups and recompute the rest. `markdown/scaling-pocs.md` §2 measured this and gated it
...
- **`cache_level` must keep working.** `test/test_sqd.py` sweeps `CACHE_LEVELS` across seven test classes,
...
`markdown/scaling-pocs.md` §2 (the partial-J gate) and §11, `markdown/wide-row-packing-session.md` §4.4.
