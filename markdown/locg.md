...
>   Both halves are now false: `test/test_ground_locg.py` exists and pins most of these items, and
...
> evenly — are why `test/test_ground_locg.py` is organized by defect. The binding invariants now
...
the module was rewritten in `8fa6be2` and `test/test_ground_locg.py` now covers most of these items
