...
**Goal:** Establish the repository's first pytest suite, covering `rqutils/ground_locg.py` with tests keyed to the defects in `markdown/locg.md`, and fix a mixed-dtype crash found while designing it.
...
- **`jax_enable_x64` must be enabled before any `rqutils` import.** Without it JAX silently yields float32/complex64 and every tolerance below is wrong. This lives in `test/conftest.py`.
...
- Create: `test/test_ground_locg.py`
...
`python_files` is explicit so collection never considers the three Jupyter notebooks in `test/` or any other non-test file there.

- [ ] **Step 2: Write `test/conftest.py`**
...
Organized by defect. Most tests correspond to a numbered item in ``markdown/locg.md``, the audit of the
...
```bash
uv run --extra test pytest test/test_ground_locg.py -v
```
...
Expected: only `test/test_ground_locg.py` items listed; no mention of `paulis.ipynb`, `qprint.ipynb`, or `sqd.ipynb`.

- [ ] **Step 6: Commit**

```bash
git add pyproject.toml test/conftest.py test/test_ground_locg.py
...
- Modify: `test/test_ground_locg.py` (append `TestEigenpair2x2`; extend the import line)

**Interfaces:**

- Consumes: `herm`, `lowest`, `rel_resid`, `symmetrize` from `conftest` (Task 1).
- Produces: `two_by_two(rng, delta_sign)` — a module-level helper in `test/test_ground_locg.py` returning a `(2, 2)` complex Hermitian `np.ndarray` whose `delta = (d0 - d1) / 2` has the requested sign. Not needed by later tasks.
- [ ] **Step 1: Extend the import in `test/test_ground_locg.py`**
...
Append to `test/test_ground_locg.py`:
...
```bash
uv run --extra test pytest test/test_ground_locg.py::TestEigenpair2x2 -v
```
...
```bash
git add test/test_ground_locg.py
...
- Modify: `test/test_ground_locg.py` (append `TestEigenpair3x3`; extend the import line)
...
```bash
uv run --extra test pytest test/test_ground_locg.py::TestEigenpair3x3 -v
```
...
- Modify: `test/test_ground_locg.py` (append `TestGroundLocg` and `TestDtypes`; extend the import line)
...
```bash
uv run --extra test pytest test/test_ground_locg.py::TestDtypes -v
```
...
`test/test_ground_locg.py` covers `rqutils/ground_locg.py` only — the other six modules have no
automated tests yet, so for those still write a throwaway `uv run python -c ...` script or run the
matching `examples/` script. `test/` also contains three Jupyter notebooks (`paulis.ipynb`,
`qprint.ipynb`, `sqd.ipynb`) used as interactive scratchpads; pytest does not collect them.

`test/conftest.py` enables `jax_enable_x64` before any `rqutils` import — every tolerance in the
suite depends on it. Tests are organized by defect and keyed to the audit items in `markdown/locg.md`;
...
```bash
git add rqutils/ground_locg.py test/test_ground_locg.py CLAUDE.md
...
**On the notebooks.** `test/*.ipynb` are scratchpads, not tests. Do not convert, delete, or collect them.
