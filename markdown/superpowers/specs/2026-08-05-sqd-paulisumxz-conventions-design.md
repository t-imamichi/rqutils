...
`markdown/locg.md` those are load-bearing and measured; only the two helpers merge.
...
**Gate:** `test/test_sqd.py` already parametrizes `CACHE_LEVELS` over all six combinations and
...
`examples/check_bench_common.py:30`, `test/test_sqd.py:100,127,130,438,460`) or passes
`add_padding=False` purely because it was the default and the test does not care
(`test/test_paulis_symplectic.py`, ~20 sites). The unpadded layout is not used for anything.
...
- `test/test_sqd.py:100,127,130,438,460` — drop the argument. Docstrings at `:9,:162` reworded.
- `test/test_paulis_symplectic.py` — drop `add_padding=False` from 28 calls.
...
- `uv run --extra dev ruff check rqutils/ test/ examples/` and `ruff format --check` clean.
- `uv run --extra dev ty check rqutils/ test/ examples/` clean.
...
- `markdown/locg.md` and `markdown/skqd.md` need no changes — items 1-4 preserve every documented invariant,
