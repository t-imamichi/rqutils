...
Establish a pytest suite in `test/`, covering `rqutils/ground_locg.py`. The repository has no
automated tests today — `test/` holds three Jupyter scratchpads (`paulis.ipynb`, `qprint.ipynb`,
...
audit that motivated that rewrite, `markdown/locg.md`, enumerates twelve specific ways the previous
...
Tests are organized **by defect**, keyed to `markdown/locg.md` items, rather than by public API surface
...
`markdown/locg.md` closes with a methodological warning that shapes this design:
...
in `test/` that is not a `test_*.py` module.
...
All in `test/test_ground_locg.py`.
...
A bug found while validating this design, not present in `markdown/locg.md`.
...
- **Sharded / multi-device execution.** `markdown/locg.md` lists this as unverified, and the
...
- **Tight spectral clusters** below ~1e-10 relative gap. `markdown/locg.md` documents the residual
