...
ones -- the shape `docs/skqd-sqd-solve-tolerance.md` §8's stated mechanism suggests after it rejected
...
Run:
    uv run python examples/scaling/poc26_warmstart.py       # demo() self-check, then xxz_rungs arm
...
def site_occupancy(states, vec):
    ...
    sum the weight of every state that has site q occupied. `docs/rqutils-requests-2.md` notes the
