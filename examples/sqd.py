...
parser.add_argument(
    "--num-paulis", metavar="NUM", type=int, default=100, help="Number of terms in the Hamiltonian."
)
parser.add_argument(
    "--subspace-frac",
    metavar="FRACTION",
...
paulis = [
    "".join("IXYZ"[ip] for ip in row)
    for row in rng.choice(4, size=(options.num_paulis, options.num_qubits))
]
coeffs = rng.uniform(-1.0, 1.0, size=options.num_paulis)
