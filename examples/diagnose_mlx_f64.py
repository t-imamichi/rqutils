...
print(
    f"ingest            : dtype={a.dtype} value_minus_1={float(a[0]) - 1.0:.6e} "
...
for name, fn in checks:
    try:
        out = fn(a)
        mx.eval(out)
        print(f"{name:<20}: dtype={out.dtype}{'  <-- LOST f64!' if out.dtype != F64 else ''}")
    except Exception as exc:  # noqa: BLE001 (probing which ops fail is this script's purpose)
...
print(f"  _project_out -> dtype={p.dtype}{'  <-- LOST f64!' if p.dtype != F64 else ''}")
...
sas = (sas + sas.T) * 0.5
mx.eval(sas)
print(f"  Rayleigh-Ritz 3x3 dtype={sas.dtype}{'  <-- LOST f64!' if sas.dtype != F64 else ''}")
theta3, kappa3 = eigenpair_3x3(sas)
mx.eval(theta3, kappa3)
print(
    f"  eigenpair_3x3 -> theta dtype={theta3.dtype} kappa dtype={kappa3.dtype}"
...
sas2 = sas_np[:2, :2]
print(
    f"  eigenpair_2x2 -> theta dtype={theta2.dtype}"
...
def sas_of(*vectors):
    ...
    rows_ = [mx.stack([mx.sum(v1 * mvs_[i2]) for i2 in range(len(vectors))]) for v1 in vectors]
    out = mx.stack(rows_)
...
pp = rc / mx.where(nr == 0.0, mx.array(1.0, nr.dtype), nr)
...
for it in range(1, 201):
    ...
    if it % 10 == 0 or it <= 3:
        ...
        print(
            f"  iter {it:>3}: |r|={float(mx.linalg.norm(rc)):.6e}  "
