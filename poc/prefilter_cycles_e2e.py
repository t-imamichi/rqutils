...
2. **It is the wrong fixture family.** prefilter_gpu uses random 100-term operators. §3.1's 27-configuration
...
the prefilter_gpu surface does not reach the default, and ``(32, 2)`` stays on §3.1's evidence. A clear
...
Run::
    ...
    uv run --extra qiskit python poc/prefilter_cycles_e2e.py
    uv run --extra qiskit python poc/prefilter_cycles_e2e.py --rounds 15
    uv run --extra qiskit python poc/prefilter_cycles_e2e.py --arms 32,2 32,4 40,2
"""
...
def xxz_strings(nq, delta, bx):
    ...
    Copied from davidson_xxz rather than imported: that module parses its own argv at import time, so
...
def xxz_krylov(nq, rungs, cap, rng):
    """States reachable from |Neel> in `rungs` nearest-neighbour hops (hash_partition's fixture, via davidson_xxz).
...
def time_one(hamiltonian, states, prefilter):
    ...
    and nothing is cached between calls: the `get_xsource` precompute that prefilter_gpu excludes is exactly
...
def main():
    ...
    print(
        "Setup is INSIDE the timed region -- that is the whole point (prefilter_gpu excludes it; "
