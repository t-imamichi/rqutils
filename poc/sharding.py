...
Run on virtual CPU devices (the default is 4, so no flag is needed)::
    ...
    uv run --extra qiskit python poc/sharding.py

Run on real GPUs, which exercises the same assertions against real kernels and interconnect::

    uv run --extra qiskit python poc/sharding.py --devices 0,1,2,3
...
addresses only its own shard (needs the ``mpi`` extra)::
    ...
    mpirun -n 4 uv run --extra mpi python poc/sharding.py --devices mpi
...
# `argparse` before `import jax`, as in gpu_unverified/prefilter_gpu/hash_partition_jax/uniquify_sharded: CUDA_VISIBLE_DEVICES and XLA_FLAGS are
...
def main():
    ...
    if ndev < 2:
        ...
        print("  uv run --extra qiskit python poc/sharding.py --host-devices 4")
        print("  uv run --extra qiskit python poc/sharding.py --devices 0,1")
        ...
        if not virtual:
            ...
            # there; gpu_unverified records the same trap. Say so rather than let the flag look broken.
            print()
