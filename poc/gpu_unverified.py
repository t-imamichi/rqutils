...
**Both sort-related claims below are measured against ``searchsorted.xsource_sort_legacy``, a pinned copy of
...
**Claim 3: real multi-GPU sharding.** ``sharding.py`` validated the sharding code paths on
...
Run on a CUDA machine:
    ...
    uv run --extra qiskit python poc/gpu_unverified.py
    # multi-GPU (needs >= 2 physical GPUs; verify with nvidia-smi -L first):
    uv run --extra qiskit python poc/gpu_unverified.py --devices 0,1,2,3
"""
...
from searchsorted import FILL_BYTE, xsource_searchsorted_u64, xsource_sort_legacy
...
def claim1_sort_leak():
    ...
    # so the arm under test is searchsorted's pinned legacy sort.
    def sweep(fn):
...
def claim2_speedup_on_gpu():
    ...
    print()
    print(
        "Baseline is searchsorted's xsource_sort_legacy, NOT rqutils.sqd.get_xsource. Commit 23fb226 made"
    )
    ...
    for num_states in sizes:
        ...
        if prev is not None:
            ...
            if grew < 0.5 * ngrew:
                ...
                # recorded rather than open: markdowns/scaling-pocs.md "Measured on GPU (NVIDIA GH200
                ...
                # 12-25x. The quotable GPU figure comes from searchsorted's check_scaling --sweep-to, which
                ...
                print(
                    ...
                    'markdowns/scaling-pocs.md "Measured on GPU"; quotable GPU number is 5.15x at '
                    "N=64M from "
                    "searchsorted.py check_scaling --sweep-to."
                )
...
def claim3_multi_gpu():
    ...
    if ndev < 2:
        ...
        print("  flag. This claim needs a physically multi-GPU machine; sharding.py already")
    ...
    print()
    print("  NOTE: sharding.py already validated sharding CORRECTNESS on virtual CPU devices")
