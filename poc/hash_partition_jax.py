...
``hash_partition.py`` validated the *algorithm* in numpy: hash each state's packed key to pick
...
**Bucketing uses ``D`` passes of an ``[n]`` cumsum, not one ``[n, D]`` one-hot.** ``range_partition`` rejected the
...
worst case: ``cap = m/D + sqrt(2*(m/D)*ln D)`` with ``m = N/D``, the balls-in-bins bound ``hash_partition``
...
``range_partition``'s phase 3, ``D`` independent ``lax.sort``s under ``vmap``. And ``uniquify_states`` remains a
separate blocker.

Run on virtual CPU devices (the default -- correctness only)::

    uv run python poc/hash_partition_jax.py

Run on real GPUs, which is the only way the timing question can be answered::

    uv run python poc/hash_partition_jax.py --devices 0,1,2,3
"""
...
def mix64(x, xnp=jnp):
    ...
    packed key. ``hash_partition`` records why the published prefix hash cannot be used -- on a 1D XXZ
...
def partition(states, num_shards):
    ...
    Host-side here. In the library this is ``range_partition``'s phase 3 -- ``D`` independent ``lax.sort``s
...
def xxz_krylov(num_qubits, max_states):
    ...
    ``hash_partition`` establishes why this fixture rather than random draws -- it is the one that exposes the
