...
**16.00x imbalance at d=16, i.e. total collapse**. On a **real 1D XXZ Krylov subspace** the failure is
worse and is not a corner case: prefix hashing measures **exactly d** at every device count for n=30
(4.00x/16.00x/64.00x/256.00x -- one shard holds everything), because ``B = 4`` bytes leaves 33 leading
zero bits and the top 12 are constant *by construction*. At n=60/100 it is still **2.05-118.98x**, since
a magnetization-conserving Krylov subspace concentrates near Neel and even the non-pad high bits barely
vary. That is the same low-entropy-high-bits failure
``range_partition.py`` recorded for equal-range splitting ("at n=100 that word is 7 bytes of
...
sorted block, a hash hands it a scattered subset. That is ``range_partition``'s phase 3 (``d`` independent
``lax.sort``s under ``vmap``, zero collectives) and runs **once at setup**, not per call.

**Range splitting is not the alternative it appears to be, and measuring it on the states hides why.**
Splitters derived from the state list balance *that list* by construction -- but what gets **routed is
the targets** ``S ^ X``. On XXZ at n=60, d=64 the targets measure up to **14.95x**, and the failure is
hop-dependent: a swap on qubits (45, 46) gives 1.01x while (0, 1) or (59, 0) gives ~14.9x, because
swapping high-order bits moves a key far in lex order and piles targets into few buckets. XXZ has a hop
on every bond, so the worst case is always present. Worse, splitters are built at setup and a real SQD
run *grows* the subspace during configuration recovery: splitters from half the subspace applied to the
full set measure **32.51x**. Whole-key hashing measures **1.03-1.05x** in every one of those cases,
because it does not depend on the population at all.
...
Run::
    ...
    uv run python poc/hash_partition.py
"""
...
def xxz_krylov(nq, rungs, cap, rng):
    """States reachable from |Neel> in ``rungs`` nearest-neighbour hops -- a real 1D XXZ subspace.

    XXZ conserves magnetization (``XX + YY`` is a hop), so the subspace lives in one Hamming-weight
    sector and is generated by repeated *local* moves from a reference state. That leaves it
    concentrated near Neel rather than spread over the weight shell, which is the structure the
    synthetic fixtures above lack and the reason prefix hashing collapses on it.
    """
...
def main():
    ...
    print(
        "   Sizing shard capacity from that bound is the same role range_partition's `slack` plays."
    )

    print("\n4. a real 1D XXZ Krylov subspace -- the case that matters")
    print(f"   {'nq':>4} {'N':>8} {'prefixes':>9} {'prefix-hash':>12} {'whole-key':>10}")
    for nq_xxz in (30, 60):
        rng = np.random.default_rng(3)
        states = xxz_krylov(nq_xxz, rungs=12, cap=60_000, rng=rng)
        keys = np.asarray(_pack_state_keys(jnp.asarray(states)))
        prefix = keys >> np.uint64(64 - 12)
        mean = len(keys) / 64
        pre = np.bincount((mix64(prefix) % np.uint64(64)).astype(np.int64), minlength=64)
        whole = np.bincount((mix64(keys) % np.uint64(64)).astype(np.int64), minlength=64)
        print(
            f"   {nq_xxz:>4} {len(keys):>8} {len(np.unique(prefix)):>9} "
            f"{pre.max() / mean:>11.2f}x {whole.max() / mean:>9.2f}x"
        )
    print("   d=64. At nq=30 the prefix imbalance is exactly d: B=4 bytes leaves 33 leading zero")
    print("   bits, so the top 12 are constant and one shard holds the whole subspace.")

    print("\n   exactness over EVERY XXZ X group (nq=60, d=64):")
    rng = np.random.default_rng(3)
    states = xxz_krylov(60, rungs=12, cap=60_000, rng=rng)
    bonds = [(q, (q + 1) % 60) for q in range(60)]
    worst, mismatches = 0.0, 0
    for a, b in bonds:
        eye = np.eye(60, dtype=np.uint8)
        xsig = np.packbits(np.concatenate([np.zeros(1, np.uint8), eye[a] + eye[b]]))
        reference = np.asarray(jax.jit(get_xsource)(xsig, jnp.asarray(states)))
        got, counts = hash_partitioned_xsource(states, xsig, 64)
        if not np.array_equal(reference, got):
            mismatches += 1
        worst = max(worst, counts.max() / (len(states) / 64))
    print(f"   {len(bonds)} hops verified, {mismatches} mismatches, worst imbalance {worst:.2f}x")
    assert mismatches == 0, "hash-partitioned result differs from get_xsource on an XXZ hop"
