...
exactly what happened on the first GPU run of ``gpu_unverified.py``. The *correctness* arms still
...
Run: uv run --extra qiskit python poc/searchsorted.py
"""
...
def xsource_sort_legacy(xsignature, states):
    ...
    **This is the baseline arm, and it lives here because the library no longer provides one.**
    ...
    measures 1.00x. That is exactly what happened on the first GPU run of ``gpu_unverified``: both arms were
...
def check_scaling(sizes=None, num_qubits=24):
    ...
    # the check that explains gpu_unverified's flat sort arm on a GH200 (1256/1313/1599 ms for 25x N) -- the
    ...
    print("  this analytic table cannot speak to it either way -- gpu_unverified.py measures")
