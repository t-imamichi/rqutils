...
state list must live on one device. ``range_partition.py`` made the *sort* shardable and named two
...
**Range partitioning, not the hashing of hash_partition/hash_partition_jax.** This function's output feeds
...
**The capacity model is NOT balls-in-bins, and that is the main lesson here.** ``hash_partition_jax`` sizes its
...
overflowed by 42,712. ``range_partition`` already had the right rule: *"slack must exceed the splitter imbalance;
...
signal that matters, which is how ``range_partition``'s ``cap`` bug shipped.
...
Run on virtual CPU devices (the default -- correctness only)::
    ...
    uv run python poc/uniquify_sharded.py

Run on real GPUs, which is the only way the routing-cost question can be answered::

    uv run python poc/uniquify_sharded.py --devices 0,1,2,3
...
this rank's when it does not, so single-process runs cannot distinguish the two::
    ...
    mpirun -n 4 uv run --extra mpi python poc/uniquify_sharded.py --devices mpi
"""

import argparse
import functools

# `argparse` before `import jax`, as in gpu_unverified/prefilter_gpu: CUDA_VISIBLE_DEVICES and XLA_FLAGS are both read
...
def ge_lex(rows, splitter):
    ...
    puts every row in one bucket. That is the same collapse ``range_partition`` documented for equal-range
...
def bucketize(vals, num_buckets, key, cap, fill, live=None):
    ...
    ``[n, num_buckets]`` one-hot: ``range_partition`` rejected the one-hot for costing ``O(N*d)`` where this is
    ``O(N)``.
...
def capacities(num_states, states_size, num_shards, slack=1.35):
    ...
    **Not balls-in-bins.** That bound assumes independent random destinations, which holds for
    ``hash_partition_jax``'s hash and fails for range splitters, whose bucket sizes are set by the data. ``range_partition``'s
...
def shard_counts():
    ...
    ``d = 1`` is skipped here, unlike ``hash_partition_jax``: with one bucket there is no second routing round to
