...
Run on a CUDA box::
    ...
    uv run --extra qiskit python poc/prefilter_gpu.py
    uv run --extra qiskit python poc/prefilter_gpu.py --devices 0,1,2,3

Across nodes, one GPU each (needs the ``mpi`` extra)::

    mpirun -n 4 uv run --extra mpi python poc/prefilter_gpu.py --devices mpi
"""
