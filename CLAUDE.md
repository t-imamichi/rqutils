...
deprecated MLX port lives under `examples/mlx/`, **including the port itself** (`solver.py`, moved
out of `rqutils/` on deprecation). Names there are **unqualified by design** — the directory already
says `mlx`, so `solver.py` and `check_solver_headless.py` rather than `ground_locg_mlx.py` and the
older `check_ground_locg_mlx_mlx.py`. The suffix instead carries the fact that actually governs how
you run a script:

| | subject | |
|---|---|---|
| `solver.py` | the deprecated MLX LOBPCG port itself | needs MLX (darwin) to import |
...
| `check_solver_headless.py` | `solver.py` via a numpy shim | headless |
| `check_solver_device.py` | `solver.py` on real MLX | **needs a Metal device** |
...
The eight scripts are a coupled set, not independent files: `bench.py`, `check_solver_headless.py`,
`check_solver_device.py` and `check_bench_common.py` all reach `_bench_common.py` through
`sys.path.insert(dirname(__file__))`; `check_solver_headless.py`/`count_ops.py` read `solver.py`'s
source text off disk (`os.path.join(HERE, "solver.py")`) and re-exec it against a numpy shim, while
`bench.py`/`check_solver_device.py` import it as a plain top-level `solver` module; and
`check_bench.py` builds a path to `bench.py` to subprocess it. Move or rename one and an import, a
`SRC` path, or that subprocess breaks — so change them together and re-run the four headless checks
afterwards.
...
after touching `sqd`; **`svsim`'s `out_sharding` is still exercised by nothing.** Note what
`poc7_sharding.py` does *not* cover: it never sets `cache_level`, so it runs the default `(1, 0)` and
never reaches the `cache_level[1] == 2` precompute. That is how `all_diagonals` shipped with unsolved
sharding while the POC stayed green — **a passing sharding POC is not evidence for a path it does not
select**, so exercise a new mesh-sensitive path directly (`XLA_FLAGS=--xla_force_host_platform_device_count=4`
plus an explicit `cache_level`) rather than inferring it from the POC. Measured while doing that: the
three `cache_level[0] == 0` levels have raised `ShardingTypeError` on any mesh since before this work,
for reasons unrelated to the diagonal path. The deprecated MLX
...
Seven largely independent modules under `rqutils/`; nothing but `sqd.py → {paulis/symplectic.py, ground_locg.py}` and `qprint.py → paulis/general.py` couples them. (The MLX port used to be an eighth; it now lives at `examples/mlx/solver.py` — see below.)
...
**`ground_locg.py`** — single-vector (block-size-1) LOBPCG specialization used as `sqd`'s eigensolver, with the Rayleigh–Ritz step solved analytically (`eigenpair_2x2`, `eigenpair_3x3` via Cardano) instead of via `eigh`, to keep memory down for huge vectors. It is sharding-transparent **only if the `mat` callable preserves output sharding** — that contract is why every `apply_*` in `sqd.py` passes `out_sharding=jax.typeof(vec).sharding`. Every guard in it is load-bearing and was measured: `docs/locg.md` catalogues seven defects (I1–I7) that each failed *silently*, returning a plausible wrong number rather than raising. Don't "simplify" the balancing, the re-orthogonalizations, or the zero-direction masks. **`docs/locg.md` is stale** — it audits the pre-rewrite module, so its line numbers, its "no pytest suite exists" scope note, and its A1–A5 gaps (all since fixed) don't apply; cite it for the I-numbers and the measurements only, and read the module docstring for what currently holds. One severity is partly retracted there: I5's full 1.0 collapse needed I4's 2000-iteration runs to develop, so the *eigenvalue* stays correct without the guard — but the guard is pinned, by `TestBasisOrthogonality`, which asserts `|<x|y>|` off the `debug=True` diagnostics and fails 3 of 4 arms when it is neutered. Two traps when editing it. `body_iter1`'s exclusion bound (`2|rho| + 1`) is **not** `body()`'s (`max(diag) + sum(|diag|) + 1`) specialized to one entry — the general form collapses to a constant `1.0` for the negative `rho` of a ground-state search. Both are *valid* (each strictly exceeds the one retained entry, so the eigensolver still can't pick the excluded slot), but the unified form's margin stops tracking the operator scale, which is a poor trade in a routine whose other guards exist because large shifts destroy precision. Don't unify without redoing the bound argument. And the one-matmul `_compute_sas` from the MLX port (`examples/mlx/solver.py`) does **not** port here: measured 98.7 ms against the scatter form's 27.7 ms at N=16.8M, because stacking three huge vectors is two 402 MB temporaries per iteration — the copy this module exists to avoid.

**The MLX port is deprecated and now lives at `examples/mlx/solver.py`** — not under `rqutils/`, not in the published API. The JAX solver measured faster even on the MLX GPU backend, so nothing should run it in preference to `ground_locg.py`; it is kept as a readable reference implementation. The **"change both" parity rule is retired**: `ground_locg.py` is free to evolve without a matching edit there. Two fused Metal kernels, the `device="cpu"|"gpu"` parameter that selected them, and the two static MSL guards that covered them were all removed on deprecation — what they were worth, the two verified *negative* results, and the cost model behind them are preserved in `docs/mlx-metal-kernels.md`, now flagged as a historical record. Read it before reviving a kernel; also revive the MSL guards, since a numpy shim never compiles MSL text.

What remains there: one op-graph path at f32 or f64, unconditional `mx.compile`, real-symmetric only (raises on complex input), and 65.0 op constructions per iteration. Its guards are still load-bearing *for that file* — don't strip them. `check_solver_headless.py` was trimmed alongside the deprecation and its coverage re-verified by reverting each guard in place: eight of ten are caught, and the two that are not (**I5**'s re-orthogonalization, **I6/I7**'s zero-direction paths) were equally uncaught before the trim — see that file's docstring, which records both gaps and why. One lesson worth keeping: the I3 rank-aware-selection guard had been covered *only* as a side effect of comparing the fused Metal eigensolve against the op-graph one, so deleting the kernel silently took its only test with it. It is now pinned directly against `numpy.eigvalsh` over 52 matrix classes. **When you delete a comparison arm, check what it was incidentally covering.** Two facts worth keeping. With `tol != 0.` the convergence test runs at `_COMPILE_CHUNK` (10) boundaries, so a solve terminating on a zeroed search direction can report an iteration count rounded up to a multiple of 10 — never wrong, and don't "fix" it by threading an OR-accumulated flag through the compiled carry (that trade was costed and declined). And when benchmarking a before/after on it, expect a **noise floor of 3.9%** on that arm: treat any difference under ~4% as unresolved.
