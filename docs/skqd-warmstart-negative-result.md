...
(`docs/skqd-sqd-solve-tolerance.md` §8); the refined shape was measured on 2026-09-17 and produced **no
...
against a dense/sparse oracle every round. Harness: `examples/scaling/poc26_warmstart.py` in `rqutils`
...
(`docs/locg-next-candidates.md` §1.4 records it as a standing constraint on any candidate).
...
`rqutils`' own `poc24_davidson_xxz.py` asserted the opposite until 2026-09-17 — true of the
...
does depend on which degenerate member gets picked — see `docs/rqutils-requests-2.md`). Measured:
...
3. **There is no seam, and `rqutils` will not add one.** `sqd()` does not accept a start vector; it is
   ...
   deterministic function of the subspace (`docs/rqutils-requests-2.md`; an early version gave 5
...
```bash
# in the rqutils repo
uv run python examples/scaling/poc26_warmstart.py          # self-check, then the hop-rung arm
uv run python -c "import examples.scaling.poc26_warmstart as m; m.run_recovery(delta=0.5)"
```
...
`docs/skqd-sqd-solve-tolerance.md` §8.
