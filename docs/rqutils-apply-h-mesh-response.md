...
Reply to `docs/rqutils-apply-h-mesh-request.md`, from the `rqutils` side. Branch `dev`, version still
...
(`tests/_sharded_apply_h_vec.py`, `committed` arm, asserted to agree with the host arm at exactly 0.0).
...
point cost a red test before it was handled; `tests/_sharded_sqd_prefilter.py` is the arm that caught it.
...
`hproj` returns a host scipy matrix, `spinchain` never calls it, and `examples/scaling/poc7_sharding.py`
