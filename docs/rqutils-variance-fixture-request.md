...
`docs/skqd-basis-opt-optimization.md` and `docs/skqd-warmstart-negative-result.md` are *outbound*:
...
this is stated as the blocking reason in `docs/rqutils-tol-response.md` §2 and left explicitly open in
`NOTES.md`. It has now blocked a second investigation (`docs/skqd-warmstart-negative-result.md`), which
...
And its cost is not where a shape-based estimate would put it. From `docs/rqutils-precond-request.md`,
...
`docs/rqutils-tol-request.md` argued that the slow draws are slow because *the last digits are
expensive*, so a residual-targeted `tol` should compress the variance. `docs/rqutils-tol-response.md` §2
...
> **Run this as a standalone sweep, not through the recovery pipeline.** `docs/skqd-sqd-solve-tolerance.md`
...
- **Variance compresses** → tolerance guidance goes into `sqd`'s docstring with these numbers, and the
  ...
  `docs/skqd-sqd-solve-tolerance.md`'s `solve_tol <= 1e-11` assertion, so the payoff is whatever
...
Either way the open claim in `NOTES.md` and `docs/rqutils-tol-response.md` §2 gets closed with a
...
- **Not a warm-start request.** See `docs/skqd-warmstart-negative-result.md` — measured 2026-09-17, no
